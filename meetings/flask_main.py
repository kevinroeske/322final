...
#        registered_users.append(profile['name'])
#    flask.session['users'] = registered_us
    return render_template('index.html')

@app.route("/show_times")
def show_times():
    email_address_list = []
    start_time_list = [flask.session['begin_time']]
    end_time_list = [flask.session['end_time']]
    master_busy_list = []
    person_list = request.values.getlist('checked')
    for person in person_list:
        for profile in get_profiles():
            if person in profile['name']:
                email_address_list.append(profile['email'])
                start_time_list.append(profile['session']['begin_time'])
                end_time_list.append(profile['session']['end_time'])
                if 'busy_blocks' in profile['session']:
                    for block in profile['session']['busy_blocks']:
                        master_busy_list.append(block)
    app.logger.debug("Busy list: " + str(master_busy_list))
    app.logger.debug("Session state: " + str(flask.session))
    app.logger.debug("start time list " + str(start_time_list))
    app.logger.debug("end time list " + str(end_time_list))
    flask.session['begin_time'] = max(start_time_list)
    flask.session['end_time'] = min(end_time_list)
    flask.g.emails = email_address_list
    flask.g.freetime = calculate_free_times.get_free_times(master_busy_list, flask.session)
...
def select():
    ...
    for profile in col:
        if profile['name'] == user_name:
            ...
            if 'busy_bocks' in profile['session']:
    ...
    for profile in get_profiles():
        registered_users.append(profile['name'])
    if state_object['name'] not in registered_users:
        registered_users.append(state_object['name'])
    new_user_object = {"type": "users", "users": registered_users }
    collection.delete_one({"type": "users"})
    collection.insert(new_user_object)
...
def people():
    other_users = []
    flask.g.url = request.url.split("people?")[0]
    user_list = collection.find({'type': 'users'})
    for entry in user_list:
        for user in entry['users']:
            if user != flask.session['user_name']:
                other_users.append(user)
        if other_users != []:
            flask.g.users = other_users
    if 'users' in flask.g:
        app.logger.debug("g variable user dump: " + str(flask.g.users))
    for profile in get_profiles():
...
#            app.logger.debug("Free times stored in cookie: " + str(flask.g.freetime))
            adjusted_profile['session']['busy_blocks'] = cooked_events
            if len(events) != 0:
                adjusted_profile['email'] = events[0]['creator']['email']
            else:
                adjusted_profile['email'] = ""
            collection.delete_one({"name": flask.session['user_name']})
            adjusted_profile['begin_time'] = flask.session['begin_time']
            adjusted_profile['end_time'] = flask.session['end_time']
...
def setrange():
    ...
    updated_session = collection.find({'name': flask.session['user_name']})
    for session in updated_session:
...
#        summary = event['summary']
        cooked.append({"date": date, "start" : start, "end": end})
